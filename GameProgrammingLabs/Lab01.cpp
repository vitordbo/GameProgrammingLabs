...
int APIENTRY WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    wndclass.hIcon = LoadIcon(NULL, IDI_QUESTION);  // b) O ícone padrão da aplicação seja um ponto de interrogação
    wndclass.hCursor = LoadCursor(NULL, IDC_WAIT);  // c) O cursor padrão da aplicação seja uma ampulheta
    wndclass.hbrBackground = (HBRUSH)GetStockObject(GRAY_BRUSH);  // a) mudando cor pra cinza
    ...
    hwnd = CreateWindow("BasicWindow",                // classe da janela
        ...
        640,                          // largura inicial => d) janela tenha um tamanho de 640x480
        480,                          // altura inicial  => d) janela tenha um tamanho de 640x480
